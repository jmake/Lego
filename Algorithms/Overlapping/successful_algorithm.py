...
def bpy_to_vtk(obj_name) :
    ...
    #vertices = np.array([vert.co[:] for vert in obj.data.vertices])
    vertices = np.array([obj.matrix_world @ vert.co for vert in obj.data.vertices])
...
def collisionDetectionB_apply(vtk_meshes) : 
    collisionDetectionB = CollisionDetectionB() 

    for k,v in vtk_meshes.items(): 
        #print(k,v)
        collisionDetectionB.ObjectAdd(k,v) 

    #collisionDetectionB.ObjetShow() 
    collisionDetectionB.Calculate() 
    collisionDetectionB.NetworkShow()
...
def usd_load(plot) :
    filepath = r"F:\Models\USD\10369b1.usd" # 4 
    filepath = r"C:\Users\zvl_2\OneDrive\Documentos\untitled.usd" # 27?
    ...
    vtks = {k:bpy_to_vtk(k) for k in meshes_lego}

    for meshj_name,vtk_mesh in vtks.items() : 
        com = GetCenterOfMassFilter(vtk_mesh) ## no conincide, checar!!
        print(meshj_name, com, vtk_mesh.GetNumberOfPoints() ) 

    if plot : 
        plotter = Plotter() 
        for meshj_name,vtk_mesh in vtks.items() : 
            outline = GetOutline(vtk_mesh, False)
            #outline = GetOBBTree(vtk_mesh, 0)
            plotter.ActorAdd(outline, colors.GetColor3d("Blue"), 1.0) 
            plotter.ActorAdd(vtk_mesh, colors.GetColor3d("Green"), 0.25) 
        plotter.Show() 
...
## X.0. 
clean_all_objects()
vtk_meshes = usd_load(False) 

## Base_535 -> Base_525 -> Base_513 -> Base_514
select = ["Base_535", "Base_525", "Base_513", "Base_512", "Base_514", "Base_538"]

vtk_meshes = {s:vtk_meshes.get(s) for s in select}
collisionDetectionB_apply(vtk_meshes) 
